...
def create_headers(code, token):
    ...
    return headers
def create_headersnewyuming(code, token):
    headers = {
...
def complete_task(code, token, tid):  # 棉花工厂
    ...
    headers = create_headersnewyuming(code, token)
    # payload = {
    #     "tid": tid,
    #     "relate_id": 0,
    # }
...
def lq_fd(code, token, tid):  # 三餐福袋和签到
    ...
    try:
        ...
        if response_data.get("code") == 200:
            ...
            print(f"{task_name}：{response_data}")
    except requests.exceptions.RequestException as e:
        print(f"请求失败: {e}")
