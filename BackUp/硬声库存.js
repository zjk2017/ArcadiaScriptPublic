/*
cron: 10 8 * * *
硬声库存  签名错误 库存接口不知道拼接规则 需要解包 可能和这两种不一样
注册登录之后，设置密码，把手机密码填到yingshengAccount里
多账户换行或者@隔开，格式：account=13888888888&password=xxxxxxxx
一天至少三次
区分域名的 第几页可以修改
ysapi.elecfans.com 用calculateSign_ysapi
/yingsheng.elecfans.com calculateSign_yingsheng  用这个
*/

const $ = new Env('硬声库存未开发完成');
...
class UserInfo {
    ...
    calculateSign_yingsheng_kucun(param) {
        param['Authorization'] = this.auth
        // param['token'] = this.auth
        param['platform'] = 'h5'
        param['timestamp'] = Math.floor(Date.now()/1000)
    ...
    calculateSign_ysapi(param) {
        ...
        param['timestamp'] = Math.floor(Date.now()/1000)
        console.log( SHA1Encrypt($.json2str(param,'&',true)))
    ...
    async getkucun() {
        ...
        try {
            let param = {'limit':6,'page':7}
            let url = `https://yingsheng.elecfans.com/ysapi/wapi/exchange/prize/allList?${$.json2str(param,'&')}`
                       console.log(url)
            let body = ``
            param = this.calculateSign_yingsheng(param)
            let urlObject = populateUrlObject(url,param,body)
            ...
            if(result.code==0) {
               let data = result.data.data
                ...
                console.log(`查询库存失败: ${result.message}`)
            }
        } catch(e) {
            console.log(e)
        } finally {}
...
function populateUrlObject_kucun(url,param,body=''){
    let host = url.replace('//','/').split('/')[1]
    let urlObject = {
        url: url,
        headers: {
