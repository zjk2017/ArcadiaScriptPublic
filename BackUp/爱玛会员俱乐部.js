...
------------------------------------------
变量名 aima
变量值 https://scrm.aimatech.com/aima 请求头Headers中access-token 、activityId的值 多账号&或换行或新建同名变量 多变量# 9月activityId是100000954 过了9月改下就行了
[Script]
http-response
...
!(async () => {
    ...
    for (let user of userCookie) {
        ...
        $.activityId = user.split(strSplitor)[1]
        $.ckStatus = true;
        await signInSearch()
    }
