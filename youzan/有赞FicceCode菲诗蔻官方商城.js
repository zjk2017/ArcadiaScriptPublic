/**
cron: 38 7 * * *
YouZanIDListbak   例如（抓checkinId 和下面的checkinId一样的 不是随便填的 示例只有两个可以继续加） [{"12063":"SKG会员商城"},{"99":"魅族商城Lite"}]
YouZan 例如（h5.youzan域名下要抓checkinId、appId、kdtId、extraData ，id随便写， token不填都行 示例只有两个可以继续加 ） [
...
有赞通用会有多个，只留一个就行（只是为了在readme里面增加条目，显示有赞有哪些可以玩的）
**/
const $ = new Env('有赞FicceCode菲诗蔻官方商城');
    ...
    for (const item of YouZan) {
      ...
      //打印商家名称
        ...
        for (const data of item.data) {
            ...
            //console.log(date); // 输出格式为：2021-02-03 19:01:18
            ...
            if (checkin.code == -1) {
                ...
                $.msg(`❌${name} 用户:${id}${date}token已过期，请重新获取`)
                 continue
            }
