 ...
 * ====================================================================================================================================
 * 感谢smallfawn yejigao_9526498
 * update 2024/7/13 ArcadiaScriptPublic  频道：https://t.me/ArcadiaScript 群组：https://t.me/ArcadiaScriptPublic 拉库https://github.com/zjk2017/ArcadiaScriptPublic
 * rt在 https://next.gacmotor.com/app/app-api/login/loginBySms 7月13 ios实测
 * 
 * 刷新token貌似没用了
 * 仅仅修复签到 其他功能功能没有
 * 
...
const $ = new Env("广汽传祺ArcadiaScriptPublic");
const notify = $.isNode() ? require('./sendNotify') : '';
//const { updateEnv11, getEnvs, updateEnv } = require("./ql")
const appVersion = "5.1.14"
...
class UserInfo {
    ...
    async mainTask () {
        ...
        // {"code":"7001","msg":"活动尚未开始或已结束","data":null,"timestamp":1720877848190,"success":false}
    ...
    // }
        ...
        try {
            let options = {
                fn: "签到前",
                method: "get",
                url: `https://next.gacmotor.com/app/app-api/common/hcRiskControl/getRiskLevelCommon?eventId=signIn`,
                headers: this._getHeaders("get"),
            }
            let { body: result } = await httpRequest(options);
            //console.log(options);
            ...
            console.log(result);
            if (result.resultCode == "0") {
                this.lastReq=result.data.requestId
                this.captchaId=result.data.captchaId
                $.log(`签到前获取数据成功 lastReq [${result.data.requestId}] captchaId [${result.data.captchaId}]   `)
            } else {
                console.log(`❌${options.fn}签到前获取数据失败`);
                console.log(JSON.stringify(result));
            }
        } catch (e) {
            console.log(e);
    ...
    async _signIn () {
        ...
        try {
            let options = {
                ...
                headers: this._getHeaders("post"),
                body: JSON.stringify({"lastReq":this.lastReq,"captchaId":this.captchaId})
            }
            let { body: result } = await httpRequest(options);
            //console.log(options);
            result = JSON.parse(result);
