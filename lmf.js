/**
 * cron: 43 7 * * *
 * export lmf='[{"id":"1","uuid":"备用变量","token":"1","userToken":"1"},{"id":"2","uuid":"2","token":"2","userToken":"2"}]'
 * 支持青龙
 * 模版改的将就用吧
 */
const $ = new Env('绿蜜蜂旧衣服回收小程序')
const lmf = ($.isNode() ? JSON.parse(process.env.lmf) : $.getjson("lmf")) || [];
let shareCodeArr = []
let token = ''
let userToken = ''
let hadayToken = ''
...
async function main() {
    ...
    for (const item of lmf) {
        id = item.id;
        token = item.token;
        userToken = item.userToken;
        uuid = item.uuid;
        let toSignInfo = await commonGet('?m=toSign')
    ...
    for (const item of lmf) {
        console.log("查询积分")
        let points = await commonGet('?m=getUserInfo')
        let phone=points.data.phone
        let amount=points.data.amount
        // console.log(`查询积分响应: ${JSON.stringify(points)}\n`)

        console.log(`用户：${phone} 拥有积分: ${amount}\n`)
        notice += `用户：${phone} 拥有积分: ${amount}\n`
    }
    if (notice) {
...
async function commonGet(url) {
    return new Promise(resolve => {
        const options = {
            url: `https://lmf.lvmifo.com/api/5dca57afa379e${url}`,
            headers : {
                 "Host": "lmf.lvmifo.com",
